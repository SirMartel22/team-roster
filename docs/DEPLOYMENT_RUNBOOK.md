...
5. Configure the variables in each `.env.example`. The auth service must use a server-side Supabase service-role key for the atomic onboarding and password-reset functions. The JWT secret must match across auth and roster services; the notification service key must match across auth, roster, and notification services. The auth service `CLIENT_URL` must point to the public client so reset links are valid.
6. Deploy notification-service, auth-service, roster-core-service, then the client.
7. Run all three services' `/health` checks and the two-tenant acceptance journey from the implementation plan, including password-reset request, delivery, confirmation, login with the new password, reset-link reuse rejection, and admin/member logout followed by rejection of the revoked JWT from both auth and roster services.
